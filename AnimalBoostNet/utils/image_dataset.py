...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        row: Any = self.df.iloc[idx]
        
        # Construct image path
        img_path: str = self.base_path + row['split'] + '/' + row['filepath']
        ...
        if self.preprocess_images:
            ...
            try:
                ...
                if self.use_exact_yolo_preprocessing:
                    ...
                    letterboxed: Any | None = self.letterbox(image=image_np)
                    
                    # Step 3: Convert to tensor and normalize to [0,1] (ToTensor does this)
                    img_tensor: torch.Tensor = transforms.ToTensor(letterboxed)
                    ...
                    img_tensor: torch.Tensor = self.transform(image)
                    img: torch.Tensor = img_tensor
                ...
                img: torch.Tensor = torch.zeros((3, 640, 640), dtype=torch.float32)
        else:
