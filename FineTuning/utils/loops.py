'''
Here we define the training, validation, and main epoch loops needed for fine-tuning.
'''
...
def train_loop(train_dataloader: DataLoader, model: nn.Module, loss_fn: nn.Module, optimizer: optim.Optimizer, epoch: int) -> Tuple[float, float]:
    '''
    This is the training loop to be used during fine-tuning.

    Inputs:
        train_dataloader: a PyTorch dataloader wrapping around the restricted image training dataset.
        model: the PyTorch module from timm to be trained.
    ...
    # put model in train mode and iterate through batches
    model.train()
...
def valid_loop(valid_dataloader: DataLoader, model: nn.Module, loss_fn: nn.Module, epoch: int) -> Tuple[float, float]:
    '''
    This is the validation loop to be used during fine-tuning.

    Inputs:
        valid_dataloader: the PyTorch DataLoader to wrap around the restricted image validation dataset.
        model: the PyTorch module from timm to be validated.
        loss_fn: the loss function to be used (Cross Entropy Loss hard-coded).
    ...
    # put model in eval mode and start loop through batches (without weight updates)
    model.eval()
    with torch.no_grad():
...
def main_loop(train_dataloader: DataLoader, valid_dataloader: DataLoader, model: nn.Module, loss_fn: nn.Module, optimizer: optim.Optimizer, num_epochs: int, run: int, save_dir: str) -> Dict[str, List]:
    ...
    Inputs:
        ...
        model: the PyTorch module from timm to be fine-tuned.
        loss_fn: the loss function to be used (Cross Entropy Loss hard-coded).
